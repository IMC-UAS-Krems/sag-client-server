...
const NavigationMenu = <T extends ValidComponent = "ul">(props: PolymorphicProps<T, NavigationMenuProps<T>>) => {
...
const NavigationMenuTrigger = <T extends ValidComponent = "button">(
  ...
  const [local, others] = splitProps(props as NavigationMenuTriggerProps, ["class", "current"]);
  return (
    <NavigationMenuPrimitive.Trigger
      class={cn(navigationMenuTriggerVariants({ current: local.current }), local.class)}
      {...others}
    />
  );
};
...
const NavigationMenuLink = <T extends ValidComponent = "a">(props: PolymorphicProps<T, NavigationMenuLinkProps<T>>) => {
